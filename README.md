# @adcalls/n8n-nodes-call-tracking

## Development
### Installing CLI
Start by installing the n8n cli `npm install --global @n8n/node-cli`.
### Building
Build the nodes inside this project: `npm run build`.
### Linting and Testing
Lint the code with `npm run lint` or `npm run lint:fix`.

To test the code run `npm run dev`.
